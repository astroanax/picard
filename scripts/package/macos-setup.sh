...
if [ -n "$PYTHON_VERSION" ]; then
  if [[ -z "$PYTHON_VARIANT" ]]; then
    # Default to use universal2 binaries
    PYTHON_VARIANT=macos11
  fi
  wget "https://www.python.org/ftp/python/${PYTHON_VERSION}/python-${PYTHON_VERSION}-${PYTHON_VARIANT}.pkg"
  sudo installer -pkg "python-${PYTHON_VERSION}-${PYTHON_VARIANT}.pkg" -target /
